...
const RamzanCalendar = () => {
  ...
  // Move adjustTime earlier and memoize to avoid potential runtime issues
  const adjustTime = useCallback((time: string, adjustmentMinutes: number): string => {
  ...
  const fetchWeather = async (cityName: string, countryName: string) => {
    try {
      ...
      const weatherData = (cityName as keyof typeof cityWeather) in cityWeather ? cityWeather[cityName as keyof typeof cityWeather] : cityWeather['Islamabad'];
  ...
  return (
    <div
      className={`
        ...
        p-4 sm:p-6
        ${darkMode
          ? 'text-white bg-gradient-to-b from-[#0b1d2a] via-[#0f2a3d] to-[#08131c]'
          : 'text-gray-800 bg-gradient-to-b from-[#f8f6f2] via-[#eef3ea] to-[#e6efe3]'}
      `}
    >
      <main className="flex-1 w-full max-w-7xl mx-auto px-2 sm:px-4 md:px-6">
        <div className="text-center mb-6">
          ...
          <div className="mt-4 mb-6 flex justify-center">
            <div className="w-full max-w-3xl h-8 md:h-16 rounded-xl
              bg-repeat-x bg-center opacity-40
              dark:bg-[radial-gradient(circle_at_center,_rgba(34,197,94,0.35)_1px,_transparent_1px)]
              bg-[radial-gradient(circle_at_center,_rgba(22,163,74,0.25)_1px,_transparent_1px)]
              bg-[length:18px_18px]
            " />
          </div>
        </div>
        ...
        <div className={`p-4 md:p-5 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm mb-6`}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="w-full sm:w-auto text-center sm:text-right sm:border-l sm:pl-6 border-gray-300 dark:border-gray-700 min-w-0">
              <div>
                {ramzanStarted ? (
                  <div>
                    {currentPrayer === 'sehri' && timeToSehri ? (
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Next: Iftar</p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Next: Sehri</p>
                      </div>
                    ) : timeToSehri ? (
                      <div>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">⏳ Time until Sehri</p>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Prepare for blessed month</p>
                      </div>
                    )}
                  </div>
                ) : (
          ...
          <div className="flex items-center gap-4 justify-center sm:justify-end">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <div className={`inline-block h-6 w-6 transform rounded-full bg-white shadow-md transition-transform duration-300 ${darkMode ? 'translate-x-6' : 'translate-x-1'}`} />
              </button>
            </div>
          </div>
        </div>
        ...
        {showCustomLocationModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className={`p-6 rounded-xl shadow-lg w-full max-w-md mx-4 sm:mx-0 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-4 md:gap-6">
            {ramzanDates.map(({ day, date, sehri, iftar }) => (
              <div
                ...
                className={`
                  ...
                  ${darkMode ? 'bg-gradient-to-br from-gray-800 to-gray-900 text-white' : 'bg-gradient-to-br from-white to-purple-100 text-gray-800'}
                  ${getAshraColor(day)}
                `}
              >
                {/* Roza Number - responsive positioning */}
                <div className="sm:absolute sm:-top-4 sm:-right-4 absolute -top-3 -right-3 bg-purple-600 text-white w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-full shadow-lg text-sm sm:text-base">
                  {day}
                </div>

                {/* Date */}
                <p className="text-sm opacity-80 mb-2 truncate">{date}</p>

                {/* Sehri & Iftar */}
                <div className="flex justify-between mt-4">
                  <div>
                    <p className="text-sm opacity-70">Sehri</p>
                    <p className="text-lg font-bold">{sehri}</p>
                  </div>

                  <div className="text-right">
                    <p className="text-sm opacity-70">Iftar</p>
                    <p className="text-lg font-bold">{iftar}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Footer */}
      <footer
        className={`mt-10 pt-6 border-t text-center text-sm px-3 sm:px-6
          ${darkMode ? 'border-green-900 text-gray-300' : 'border-green-200 text-gray-600'}
        `}
      >
        <p className={`
        ...
        <div className="mt-2 space-y-1">
          <p>
            📞 <span className="font-medium">Phone:</span> +92 310 348 5834
          </p>
          <p>
            ✉️ <span className="font-medium">Email:</span> gcefbarea15@gmail.com
          </p>
        </div>

        <p className="mt-4 text-sm opacity-70">
          © {year} — Ramzan Calendar
        </p>

        <p className="mt-1 text-sm opacity-80">
          Created by{" "}
          <a
            href="https://www.linkedin.com/in/muhammad-asaad-aslam/"
            target="_blank"
            rel="noopener noreferrer"
            className={`font-medium hover:underline ${darkMode ? 'text-red-400' : 'text-red-700'}`}
          >
            Muhammad Asaad
          </a>
        </p>
